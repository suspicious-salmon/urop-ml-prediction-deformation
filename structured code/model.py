...
class ResBlock(nn.Module):
    def __init__(self, in_dim, n_input_channels=1, do_batchnorm=False, do_layernorm=False, do_residual=True, act=nn.ReLU()):
        ...
        self.conv1 = nn.Conv2d(n_input_channels, n_input_channels, kernel_size=3, padding="same", bias=do_bias)

        self.bn2 = nn.BatchNorm2d(n_input_channels)
        self.ln2 = nn.LayerNorm((n_input_channels, in_dim, in_dim))
    ...
    def forward(self, x):
        ...
        if self.do_layernorm: e1 = self.ln2(e1)
        e2 = self.conv2(self.act(e1))
        if self.do_residual: e2 += x
...
class UpResBlock(nn.Module):
    ...
    def forward(self, x, in_skip_x):
        u = self.upconv(x)
        c = torch.cat([u, in_skip_x])
        e = self.resblock.forward(c)
        return e

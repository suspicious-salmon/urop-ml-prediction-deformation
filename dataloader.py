...
class PyramidDataset(VisionDataset):
    def __init__(self, feature_root, label_root, scales, in_dim, out_dim, crop_amount=0, transform=None):
        ...
        for feature_name, label_name in zip(feature_names, label_names):
            ...
            assert feature.shape == in_dim and label.shape == in_dim, f"Image dimensions need to be {in_dim} for both feature and label, but were {feature.shape} for feature and {label.shape} for label."
            ...
            feature = cv2.resize(feature, out_dim, interpolation=cv2.INTER_NEAREST)
            label = cv2.resize(label, out_dim, interpolation=cv2.INTER_NEAREST)
    ...
    def __getitem__(self, index):
        ...
        feature_pyramid = [torch.from_numpy(f.astype(np.float32)/255).unsqueeze(1) for f in feature_pyramid]
        label_pyramid = [torch.from_numpy(l.astype(np.float32)/255).unsqueeze(1) for l in label_pyramid]

        return feature_pyramid, label_pyramid
    
class RandomNoise(VisionDataset):
    def __init__(self):
        super().__init__(None)
        self.noise = torch.from_numpy(u.readim("noise64.png", cv2.IMREAD_GRAYSCALE).astype(np.float32)/255)
...
class MemoriseShape(VisionDataset):
    def __init__(self, in_dim):
        super().__init__(None)
        self.feature = u.readim("input2_filled.png", cv2.IMREAD_GRAYSCALE)
        self.label = u.readim("output2_filled.png", cv2.IMREAD_GRAYSCALE)

        self.feature = cv2.resize(self.feature, in_dim, interpolation=cv2.INTER_NEAREST)
        self.label = cv2.resize(self.label, in_dim, interpolation=cv2.INTER_NEAREST)
        ...
        self.label = torch.from_numpy(self.label.astype(np.float32)/255)
...
class DeformedDataset(VisionDataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(feature.astype(np.float32)/255).unsqueeze(0), torch.from_numpy(label.astype(np.float32)/255)